...
def save_to_csv(seq_data, combined_seq_data, append=False):
    ...
    header = not append
    seq_df.to_csv('dataset/Biosnap_cluster/Alpha_seq.csv', mode=mode, index=False, header=header)
    combined_seq_df.to_csv('dataset/Biosnap_cluster/Alpha_stru_seq.csv', mode=mode, index=False, header=header)

# Process batch of CIF files
directory = "/home/hadmin/24zhangzhijun/FusionDTI-main/dataset/Biosnap_cluster/cif"
